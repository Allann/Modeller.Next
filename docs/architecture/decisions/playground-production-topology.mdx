...
- [ ] Enable Vercel Analytics/Speed Insights for both projects (`modeller-api`, `modeller.website`)
      plus the new `apps/studio` project. Analytics is currently unenabled on `modeller.website`:
      its injected script 404s on every page load. Per-project cost/usage alerts are not
      configured — Spend Management requires the Pro plan, which none of the three projects is on;
      Hobby's automatic usage-cap pause is the accepted substitute (see Monitoring, budgets, and
      privacy).
