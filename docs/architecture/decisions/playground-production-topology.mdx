...
(`src/Modeller.Api/appsettings.Production.json`) already covers `https://modeller-next.vercel.app`
and `https://modeller.website` — since the playground is now same-origin with `modeller.website`
from the browser's perspective, **no new CORS entry is needed** (this is one of the concrete
benefits of the path-proxy approach over the subdomain one).
...
- [ ] Enable Vercel Analytics/Speed Insights and set per-project cost/usage alerts for both projects
      (`modeller-api`, `modeller.website`) plus the new `apps/studio` project.
- [ ] No DNS or CORS changes needed — `/playground` is served from the existing `modeller.website`
      domain via the rewrite, and that origin is already in `Modeller.Api`'s `Cors:AllowedOrigins`.
