...
Agent__RequireRequestApiKey=true
Agent__TimeoutSeconds=30
Agent__MaxOutputTokens=1200
Agent__MaxPromptCharacters=24000
```

The public cockpit requires each Facilitator to enter their own Vercel AI Gateway API key. The page keeps it only in component memory and sends it in the `X-Agent-Api-Key` header for explicit AI actions. The API forwards it as authorization and does not put it in Redis, logs, analytics, URLs, or the Initiative record.

Do not configure `Agent__ApiKey`, `AI_GATEWAY_API_KEY`, or project OIDC as a fallback on the public deployment. That would let anonymous visitors spend the project's credits. For a private deployment, set `Agent__RequireRequestApiKey=false` and provide a server-side key only after access control and rate limits are in place.
...
3. Confirm `Agent__BaseUrl`, `Agent__Model`, and `Agent__RequireRequestApiKey=true`.
