...
Agent__FreeModel=alibaba/qwen3.8-27b
Agent__TimeoutSeconds=30
Agent__MaxOutputTokens=1200
Agent__MaxPromptCharacters=24000
```

The public cockpit uses `alibaba/qwen3.8-27b` without a caller key only while Vercel lists that exact model as free. The API authenticates those requests with the deployment's short-lived OIDC token and does not let the browser select a different model.
...
3. Confirm `Agent__BaseUrl`, `Agent__Model`, and `Agent__FreeModel`.
